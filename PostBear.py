...
class PBpost:
    ...
    async def check_exists(self):
        if self.id in self.bear.dld_posts:
            return 1
        res = await self.bear.db.execute("SELECT shortcode FROM posts WHERE shortcode=%s",self.shortcode)
        if res:
            self.bear.dld_posts.append(self.shortcode)
            return 1
        ...
        if not await self.check_exists():
            async with self.bear.client.get(self.url) as res:
                if res.status==200:
                    ...
                    await self.bear.db.execute("INSERT INTO posts(user_id,id,shortcode,caption,ext,timestamp,media) values(%s,%s,%s,%s,%s,%s,%s)",(self.user_id,self.id,self.shortcode,self.caption,self.ext,self.timestamp,media,))
                    self.bear.dld_posts.append(self.shortcode)
                    return 1
                else:
            ...
            for i in res:
                async with self.client.get(queries.POSTS.format(dumps({'id':i[0],'first':1}))) as r:
                    ...
                    for i,v in enumerate(self.users):
                        if v.id == i[0]:
                            self.users[i] = PBuser(r,self)
                            await self.users[i].fetch_more()
                        else:
                            self.users.append(PBuser(r,self))
                            await self.users[-1].fetch_more()
                await sleep(2,loop=self.client.loop)

    async def _save_stories(self):
        for i in self.users:
            count = 0
            for ii in i.posts:
                count+=(await ii.download())
                await sleep(1,loop=self.client.loop)
            self.info("Saved {} posts by {}".format(count,i.id))
            await sleep(3,loop=self.client.loop)
    ...
    async def start(self):
        ...
        while True:
            try:
                ...
                await self._save_stories()
            except Exception as e:
                self.error(str(e))
                #print(traceback.format_exc())
            await sleep(30,loop=self.client.loop)